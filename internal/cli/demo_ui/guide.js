...
document.querySelectorAll('.document pre > code').forEach(code => {
  ...
  label.textContent = language ? language.slice(9).toUpperCase() : 'TEXT';
  const button = document.createElement('button');
  button.type = 'button'; button.textContent = 'Copy'; button.setAttribute('aria-label', 'Copy code');
...
const headingObserver = new IntersectionObserver(entries => {
  ...
  if (!heading) return;
  pageLinks.forEach(link => link.classList.toggle('active', link.hash === '#' + heading.target.id));
