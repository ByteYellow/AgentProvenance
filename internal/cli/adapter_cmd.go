...
func adapterCmd() *cobra.Command {
	...
	list := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			fmt.Fprintln(w, commandText(cmd, "KIND\tNAME\tSTATUS\tAVAILABLE\tCAPABILITIES\tIDENTITY_KEYS\tQBS_IMPACT"))
			for _, item := range items {
				fmt.Fprintf(w, "%s\t%s\t%s\t%t\t%s\t%s\t%s\n",
					item.Kind, item.Name, item.Status, item.Available, capabilitySummary(item.Capabilities), strings.Join(item.IdentityKeys, ","), strings.Join(commandTexts(cmd, item.QBSImpact), ","))
			}
			return w.Flush()
		},
	}
	...
	inspect := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			fmt.Fprintf(cmd.OutOrStdout(), commandText(cmd, "name=%s\nkind=%s\nstatus=%s\navailable=%t\nboundary=%s\ninputs=%s\noutputs=%s\nidentity_keys=%s\nqbs_impact=%s\nnotes=%s\n"),
				item.Name, item.Kind, item.Status, item.Available, commandText(cmd, item.Boundary), strings.Join(item.Inputs, ","), strings.Join(item.Outputs, ","), strings.Join(item.IdentityKeys, ","), strings.Join(commandTexts(cmd, item.QBSImpact), ","), commandText(cmd, item.Notes))
			for _, cap := range item.Capabilities {
				fmt.Fprintf(cmd.OutOrStdout(), commandText(cmd, "capability=%s supported=%t level=%s notes=%q\n"), cap.Name, cap.Supported, cap.Level, commandText(cmd, cap.Notes))
			}
			return nil
		},
	}
