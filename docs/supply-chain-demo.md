...
From the extracted [portable release](releases/v0.8.2-rc.2.md), run
`./agentprov demo snake-supply-chain`. Signature verification, import, Run/lens
selection and temporary storage are handled automatically. Use
`./agentprov demo` to open all demos and their formatted guides. The manual
commands below are for keeping a persistent investigation store.
