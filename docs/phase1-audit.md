...
| External evaluator protocol exists | `signal context --run` emits `agentprovenance.eval_context/v1`; `signal run --external` passes that context to an external process over stdin; `signal import` validates returned `EvalSignal` records. `scripts/accept_signal_engine.sh` verifies built-in signals, Python evaluator execution, and imported reward/dataset/quality signals. AgentProvenance owns the evidence protocol, not reward or ranking policy. |
