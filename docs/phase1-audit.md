...
| Daemon API boundary exists | `agentprov daemon serve` exposes HTTP endpoints for ToolCallScope binding, paged telemetry event query, telemetry event window query, telemetry correlation explain, observability summary, timeline query, Falco ingest/spool, graph explain/verify, security risk/deviation/response query, evidence manifest materialization, forensics export, and evaluator context/import APIs. `scripts/accept_daemon_evidence_api.sh` verifies the same risk/evidence/query path through daemon APIs and CLI daemon-client mode, including `observe summary`, `timeline`, `telemetry correlations`, `graph explain`, `graph verify`, `security risks/deviations/responses`, `evidence manifest`, `forensics export`, and `signal`. |
...
| 100k telemetry pressure boundary is covered | `scripts/accept_telemetry_100k_pressure.sh` generates 100k Falco events, enqueues through daemon spool, confirms control API responsiveness while queued, drains the batch, and verifies paged query output remains bounded. Receiver row details are capped with `row_results_truncated` while summary counts remain complete. |
| Telemetry event windows exist | JSONL/Falco ingest rebuilds 10s/60s `telemetry_event_windows` grouped by run/session/tool_call/source/event_type with event, resolved, unresolved, and high-risk counts. `telemetry windows` and daemon `GET /v1/telemetry/windows` expose the same JSON schema, and `observe summary` embeds the 60s aggregate. `scripts/accept_telemetry_event_windows.sh` and `scripts/accept_daemon_evidence_api.sh` verify the path. |
...
| External evaluator protocol exists | `signal context --run` emits `agentprovenance.eval_context/v1`; `signal batch-context` emits EvalContext JSONL for batch/shard/run-list consumers; `signal run --external` passes one context to an external process over stdin; `signal import` validates returned `EvalSignal` records. Daemon mode exposes context export, built-in smoke signals, and signal import without remote shell execution. Acceptance verifies built-in signals, Python evaluator execution, batch context export, imported reward/dataset/quality signals, and daemon API coverage. AgentProvenance owns the evidence protocol, not reward or ranking policy. |
