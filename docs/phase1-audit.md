...
| External evaluator protocol exists | `signal context --run` emits `agentprovenance.eval_context/v1`; `signal run --external` passes that context to an external process over stdin; `signal import` validates returned `EvalSignal` records. Daemon mode exposes context export, built-in smoke signals, and signal import without remote shell execution. `scripts/accept_signal_engine.sh` and `scripts/accept_daemon_evidence_api.sh` verify built-in signals, Python evaluator execution, imported reward/dataset/quality signals, and daemon API coverage. AgentProvenance owns the evidence protocol, not reward or ranking policy. |
