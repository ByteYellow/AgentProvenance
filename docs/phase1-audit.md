...
| Telemetry event windows exist | JSONL/Falco ingest rebuilds 10s/60s `telemetry_event_windows` grouped by run/session/tool_call/source/event_type with event, resolved, unresolved, and high-risk counts. `scripts/accept_telemetry_event_windows.sh` verifies the path from Falco ingest to window query JSON. |
