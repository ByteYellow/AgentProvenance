...
[v0.8.2 正式版](releases/v0.8.2.md)提供 Linux 和 macOS 的 amd64、arm64 命令行包。`agentprov demo` 打开本地示例首页，包含六份经过验证的签名记录和两个可选评估器指南。阅读页支持目录、图片、表格和代码复制，并与 Dashboard 保持一致的视觉风格。
