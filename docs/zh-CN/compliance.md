...
| 状态 | 含义 |
|---|---|
| `enforced` | 映射规则已触发，并通过 deny、quarantine 或 kill 阻止相关操作。 |
| `detected` | 映射规则在检测模式下触发；操作已被观察到，但未被阻止。 |
...
`agentprovenance.compliance_rule_mapping/v1` 报告按控制项列出 `rules`、各规则的 `hits`、具体 `evidence_refs`、缺口 `gap`、建议 `recommended_next_step` 和原因 `reason`。命中记录包含决策、时间、来源事件及本次命中是否执行了阻止操作。JSON 使用 `control_id` 标识控制项，当前报告不附带 `item_id` 别名。
