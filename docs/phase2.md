...
| External evaluator protocol | Evaluator/RL/dataset consumers need programmable signals rather than YAML runtime policy decisions, but AgentProvenance should not own their reward policy. | Improved. `signal context --run` emits `agentprovenance.eval_context/v1`; `signal run --external` passes that context to an external process over stdin; `signal import` validates returned `EvalSignal` records. Built-in signals remain available for smoke coverage. `python/agentprov_eval` provides a thin SDK, and `scripts/accept_signal_engine.sh` verifies built-in output, external Python evaluator execution, and imported signal validation. |
