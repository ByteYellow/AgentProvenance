...
| Execution timeline | Users can inspect a time-ordered view of application context, runtime telemetry, evidence, policy decisions, risk signals, baseline deviations, response actions, and external effects. | MVP done. `agentprov timeline --run <run_id>` emits a human table, and `--json` emits `agentprovenance.timeline/v1` with tool-call/process/type filters plus `result_set_id` and `page_hash` for UI/API integrity checks. |
| Observability query integrity | Run summary, coverage, tool-call scope, event detail, process detail, and event-flow views need stable machine-readable integrity metadata for UI caching, export, and audit. | MVP done. `observe summary/coverage/scopes/event/process/flow --json` emit `result_set_id` and `page_hash`, aligned with timeline and graph explain query integrity. |
