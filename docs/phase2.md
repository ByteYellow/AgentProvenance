...
| Execution timeline | Users can inspect a time-ordered view of application context, runtime telemetry, evidence, policy decisions, risk signals, baseline deviations, response actions, and external effects. | MVP done. `agentprov timeline --run <run_id>` emits a human table, and `--json` emits `agentprovenance.timeline/v1` with tool-call/process/type filters for UI consumption. |
