...
| Code Signal Engine | Evaluator/RL/dataset consumers need programmable signals rather than YAML runtime policy decisions. | Initial. `internal/signal` defines registered code evaluators over trajectory/provenance evidence and `agentprov signal run --run --json` emits `agentprovenance.eval_signals/v1` with reward features, penalties, dataset labels, quality signals, result hash, and page hash. `scripts/accept_signal_engine.sh` verifies CLI output. |
