...
| Observability query integrity | Run summary, coverage, and tool-call scope views need stable machine-readable integrity metadata for UI caching, export, and audit. | MVP done. `observe summary/coverage/scopes --json` emit `result_set_id` and `page_hash`, aligned with timeline and graph explain query integrity. |
