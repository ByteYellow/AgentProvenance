...
| Evidence manifest | A run needs one stable audit entry point that references observability, timeline, objects, risk, response, and next drill-down commands without forcing a consumer to call every surface first. | MVP done. `agentprov evidence manifest --run --json` emits `agentprovenance.evidence_manifest/v1` with summary, timeline, object-list, risk-report, and response-report hashes, object type counts, security counts, and recommended drill-down queries. |
...
- `audit_manifest`
- `record_manifest`
- `evidence_manifest` as a query-level manifest, not yet persisted as a content-addressed object
