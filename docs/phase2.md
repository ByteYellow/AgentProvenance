...
| Security evidence | Risk signals, baseline deviations, and response actions are first-class graph objects and queryable records rather than side effects hidden inside policy output. | Improved. Policy-derived risk/response records are queryable, and baseline checks now compare process/file/network/risk/runtime feature vectors, persist deviation records, and create baseline-derived risk signals. `agentprov security risks/deviations/responses --json` now emits schema versions, result/page hashes, and drill-down refs back to event/process/timeline/explain views. |
