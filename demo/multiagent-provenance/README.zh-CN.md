...
本次采集只记录行为，没有在工具执行前进行阻断。尝试 A 中，`recon` 拒绝了指令；策略也可将读取凭据或访问元数据地址标为风险。尝试 B 的安装调用被允许，异常行为由传感器在执行时记录。页面中的策略结论不能证明当时实施了阻断。

### 尝试 A 没有内核覆盖

`recon` 运行时未启用 eBPF。因此，尝试 A 只有应用侧的拒绝记录，没有系统调用证据。原记录中模型确实返回了拒绝；但如果重新运行时模型接受指令，内核行为仍可能发生在采集范围之外。不能根据“图中没有外发事件”判断本分支一定没有外发。
