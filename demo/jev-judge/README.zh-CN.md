...
这张截图使用离线合成的模型回答和模拟校验结果，不代表真实 Jev 调用或人工批准。下方历史实验的英文截图和数据继续保留。
...
文件访问和 `connect` 支持“存在可疑传输”的判断，不能证明秘密字节已经发出或被接收；源记录没有采集对应外发正文。

没有运行时观测时，运行时分类必须为 `unknown`。两版均由同一固定规则处理此情况。规则纠正的结果不能计入模型能力或候选标准的改善。

源证据包导入时会验证签名；该签名不覆盖后续 Jev 结果，也不证明采集完整或模型结论正确。
