...
DATA_DIR="${DATA_DIR:-.agentprov-demo-50}"
...
AGENTPROV_BURST_MAX_INFLIGHT="$BURST_MAX_INFLIGHT" "$AGENTPROV" --data-dir "$DATA_DIR" daemon serve --listen "$LISTEN" --sample-interval 0 >"$DATA_DIR/daemon.log" 2>&1 &
daemon_pid=$!
...
cleanup() {
  for session_id in "${session_ids[@]:-}"; do
    "$AGENTPROV" --daemon-url "$DAEMON_URL" session rm "$session_id" >/dev/null 2>&1 || true
  done
...
for i in $(seq 1 "$SESSIONS"); do
  lease_id=$("$AGENTPROV" --daemon-url "$DAEMON_URL" lease create --task examples/tasks/bugfix.yaml)
  session_id=$("$AGENTPROV" --daemon-url "$DAEMON_URL" session create --lease "$lease_id")
...
for session_id in "${session_ids[@]}"; do
  (
    if "$AGENTPROV" --daemon-url "$DAEMON_URL" exec "$session_id" -- sh -lc 'printf ok >/workspace/result.txt; sleep 2' >/dev/null 2>"$DATA_DIR/results/$session_id.err"; then
...
echo "sampling_cost"
for session_id in "${session_ids[@]}"; do
  "$AGENTPROV" --data-dir "$DATA_DIR" cost sample "$session_id" >/dev/null || true
done

AGENTPROV_BURST_MAX_INFLIGHT="$BURST_MAX_INFLIGHT" "$AGENTPROV" --daemon-url "$DAEMON_URL" scheduler status
"$AGENTPROV" --data-dir "$DATA_DIR" cost show run-demo-bugfix
"$AGENTPROV" --data-dir "$DATA_DIR" telemetry list --run run-demo-bugfix --type burst_reject
if [[ "$rejected" != "0" ]]; then
