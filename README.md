...
| Signed evidence attestation | `internal/attest` signs an in-toto v1 Statement over an evidence digest inside a DSSE envelope (ed25519), the standard in-toto/SLSA transport. `forensics export` produces a `.dsse.json` attestation beside the bundle when a signing key is configured; `VerifyBundleAttestation` detects post-signing rewrites of the bundle on disk -- tamper-evidence the plain sha256 recompute cannot provide. Local key today; TPM/KMS/transparency-log anchoring is a deployment layer over the same wire format |
