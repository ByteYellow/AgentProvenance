...
| Daemon API boundary | `agentprov daemon serve` exposes core evidence-infra APIs for ToolCallScope binding, paged telemetry event query, observability summary, timeline query, graph explain/verify, evidence manifest materialization, forensics export, and evaluator context/import APIs. CLI daemon-client mode covers `observe summary`, `timeline`, `graph explain`, `graph verify`, `evidence manifest`, `forensics export`, and `signal` paths |
